...
public class WorldFrame<T> extends JFrame {
	...
	private void makeMenus() {
		...
		JMenuItem saveMap;
		menu.add(saveMap = makeMenuItem("menu.map.save", new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				FileDialog d = new FileDialog(new Frame(), "Save map to file",
						FileDialog.SAVE);
				d.setFile("*.txt");
				d.setDirectory(".");
				d.show();
				...
				try {
					...
					StringBuffer stringBuffer = saveMap();
		...
		menuItemsDisabledDuringRun.add(saveMap);
	...
	private StringBuffer saveMap() {
		...
		for (Location loc : gr.getOccupiedLocations()) {
			...
			sBuffer.append(actor.getClass().getName() + "\tlocation="
					+ actor.getLocation() + "\tdirection="
					+ actor.getDirection() + "\tcolor=" + actor.getColor()
					+ "\n");
		}
		return sBuffer;
	}
				...
				if (gr.getNumCols() == -1 && gr.getNumRows() == -1) {
					...
					JOptionPane
							.showMessageDialog(null,
									"Please select the unbounded grid to load this map.");
				}
			} else {
				if (gr.getNumCols() == -1 && gr.getNumRows() == -1) {
					JOptionPane
							.showMessageDialog(null,
									"Please select the bounded grid(10,10) to load this map.");
				} else {
					canLoad = true;
				}
			}
